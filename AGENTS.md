...
python-opinet-api는 한국석유공사 오피넷(Opinet) 오픈 API의 비공식 Python 클라이언트 라이브러리다. 공식 오픈 API 페이지에 등재된 5개 엔드포인트는 `OpinetClient`로, PDF 가이드북의 미검증 17개 엔드포인트는 `opinet.experimental.OpinetExperimentalClient`로 분리해 제공한다. 좌표(KATEC ↔ WGS84) 변환과 응답 데이터의 Python 네이티브 타입 변환을 함께 처리하는 것이 이 패키지의 핵심 책임이다.
...
- HTTP transport는 `httpx.AsyncClient` 기반이며 AsyncHttpxTransport 하나만 유지한다.
