...
async def test_invalid_key_body_maps_to_auth(load_fixture, mock_opinet):
...
async def test_limit_body_maps_to_rate_limit(load_fixture, mock_opinet):
...
async def test_401_and_403_map_to_auth(mock_opinet):
...
async def test_429_does_not_retry(mock_opinet):
...
async def test_5xx_retries_then_succeeds(load_fixture, mock_opinet):
...
async def test_5xx_final_failure(mock_opinet):
...
async def test_network_error_retries_then_raises(mock_opinet):
...
async def test_json_parse_failure_maps_to_server_error(mock_opinet):
