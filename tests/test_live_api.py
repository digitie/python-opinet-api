...
async def _retry_live_call(call):
...
async def test_live_area_code_endpoint_is_reachable(live_client):
    """라이브 서버는 문서화된 RESULT/OIL JSON 감싼 구조를 반환해야 한다."""
    data = (await live_client._require_http().get("areaCode.do"))
...
async def test_live_resolve_opinet_sigungu_code_with_vworld(live_client, live_vworld_client):
    """오피넷 시군구 코드는 VWorld district 검색으로 법정동 시군구 코드에 매핑된다."""
    areas = (await live_client.get_area_codes())
    _skip_if_empty(areas, "areaCode.do")

    mapping = (await _retry_live_call(
...
async def test_live_official_endpoints_parse_when_key_returns_data(live_client):
    ...
    averages = (await live_client.get_national_average_price())
    ...
    lowest = (await live_client.get_lowest_price_top20(ProductCode.GASOLINE, cnt=1))
    ...
    nearby = (await live_client.search_stations_around(
        lon=127.0276,
        lat=37.4979,
        radius_m=1000,
